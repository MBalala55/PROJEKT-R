...
(
  ...
  lokacija TEXT,
  zadnji_pregled TEXT  -- ISO 8601 format - last check date synced from server
);

CREATE TABLE Polje
(
  id_polje INTEGER NOT NULL,
  nap_razina REAL NOT NULL,
  ozn_vr_polje TEXT NOT NULL,
  naz_polje TEXT NOT NULL,
  id_postr INTEGER NOT NULL,
  PRIMARY KEY(id_polje),
  FOREIGN KEY (id_postr) REFERENCES Postrojenje(id_postr) ON UPDATE NO ACTION ON DELETE CASCADE,
...
(
  ...
  FOREIGN KEY (id_postr) REFERENCES Postrojenje(id_postr) ON UPDATE NO ACTION ON DELETE CASCADE,
  FOREIGN KEY (id_polje) REFERENCES Polje(id_polje) ON UPDATE NO ACTION ON DELETE SET NULL,
  FOREIGN KEY (id_vr_ured) REFERENCES VrstaUredaja(id_vr_ured) ON UPDATE NO ACTION ON DELETE CASCADE
);
...
(
  ...
  id_postr INTEGER NOT NULL,
  FOREIGN KEY (id_korisnika) REFERENCES Korisnik(id_korisnika) ON UPDATE NO ACTION ON DELETE CASCADE,
  FOREIGN KEY (id_postr) REFERENCES Postrojenje(id_postr) ON UPDATE NO ACTION ON DELETE CASCADE,
...
(
  ...
  obavezan INTEGER NOT NULL,  -- 0 = FALSE, 1 = TRUE (no DEFAULT - Room doesn't expect it)
...
(
  ...
  FOREIGN KEY (id_preg) REFERENCES Pregled(id_preg) ON UPDATE NO ACTION ON DELETE CASCADE,
  FOREIGN KEY (id_ured) REFERENCES Uredaj(id_ured) ON UPDATE NO ACTION ON DELETE CASCADE,
  FOREIGN KEY (id_parametra) REFERENCES ParametarProvjere(id_parametra) ON UPDATE NO ACTION ON DELETE CASCADE,
...
-- Indices removed: Room Entities don't define them, so schema must match
